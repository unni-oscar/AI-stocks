...
type SortField = 'symbol' | 'current_price' | 'change_percent' | 'change_absolute';
type SortOrder = 'asc' | 'desc';

const WeekHighPage: React.FC = () => {
  const [data, setData] = useState<Stock52WeekHigh[]>([]);
  ...
  useEffect(() => {
    const fetch52WeekHigh = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3035/api/analysis/52-week-high?${params}`);
        const result: ApiResponse = await response.json();
        
        if (result.status === 'success') {
          console.log('52 Week High API Response:', result.data.stocks.slice(0, 3)); // Debug first 3 stocks
          ...
          toast.error('Failed to fetch 52 week high data');
        }
      } catch (error) {
        console.error('Error fetching 52 week high:', error);
        toast.error('Error fetching 52 week high data');
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    // Default should match backend sorting (symbol asc)
    if (sortField !== 'symbol' || sortOrder !== 'asc') {
      filtered.sort((a, b) => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-lg text-gray-600">Loading 52 week high...</div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mt-4 mb-6">
        <h1 className="text-3xl text-gray-900">52 Week High</h1>
        <p className="text-gray-500 text-base mt-1">
          Stocks with their 52-week high prices from {fiftyTwoWeeksAgo} to {latestDate}.
          {dateAdjusted && (
