...
const MostActivePage: React.FC = () => {
  const [data, setData] = useState<MostActiveStock[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [searchTerm, setSearchTerm] = useState('');
  const [sortField, setSortField] = useState<SortField>('current_volume');
    ...
    const fetchMostActive = async () => {
      try {
        ...
        if (result.status === 'success') {
          ...
          toast.error('Failed to fetch most active data');
        }
      } catch (error) {
        console.error('Error fetching most active:', error);
        toast.error('Error fetching most active data');
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    // Default should match backend sorting (current_volume desc - highest volume first)
    if (sortField !== 'current_volume' || sortOrder !== 'desc') {
      filtered.sort((a, b) => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-lg text-gray-600">Loading most active stocks...</div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mt-4 mb-6">
        <h1 className="text-3xl text-gray-900">Most Active</h1>
        <p className="text-gray-500 text-base mt-1">
          Stocks with the highest trading volume on {latestDate}.
          {dateAdjusted && (
