...
const TopLosersPage: React.FC = () => {
  const [searchTerm, setSearchTerm] = useState('');
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(10);
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<TopLoser[]>([]);
  ...
  useEffect(() => {
    const fetchTopLosers = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:3035/api/analysis/top-losers?${params}`);
        const result: ApiResponse = await response.json();
        
        if (result.status === 'success') {
          console.log('Top Losers API Response:', result.data.stocks.slice(0, 3)); // Debug first 3 stocks
          console.log('RADIOCITY data:', result.data.stocks.find(s => s.symbol === 'RADIOCITY')); // Debug specific stock
          ...
          toast.error('Failed to fetch top losers data');
        }
      } catch (error) {
        console.error('Error fetching top losers:', error);
        toast.error('Error fetching top losers data');
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredAndSortedData = useMemo(() => {
    ...
    // Default should match backend sorting (price_change_percent asc)
    if (sortField !== 'price_change_percent' || sortOrder !== 'asc') {
      filtered.sort((a, b) => {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-lg text-gray-600">Loading top losers...</div>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mt-4 mb-6">
        <h1 className="text-3xl text-gray-900">Top Losers</h1>
        <p className="text-gray-500 text-base mt-1">
          Stocks that have decreased in price from {previousDate} to {latestDate}.
          {dateAdjusted && (
              ...
              currentData.map((stock, idx) => (
                <tr key={`${stock.symbol}-${idx}`} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-right">
                    <span className="inline-block px-2 py-1 rounded-full text-xs bg-red-100 text-red-700">
              ...
              <>
                ...
                {currentPage > 3 && (
                  <button
                    ...
                    className="w-7 h-7 flex items-center justify-center rounded border border-red-200 bg-red-50 text-red-700 font-semibold text-sm"
                  >
                    {currentPage}
                  </button>
                )}
